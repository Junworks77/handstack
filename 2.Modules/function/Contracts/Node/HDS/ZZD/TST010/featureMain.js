...
module.exports = {
	GF01: async (dynamicParameters, dataContext) => {
        var applicationID = 'HDS'; // JSON.parse(dynamicParameters)[0].Value;
		var awaitMS = 25; // parseInt(JSON.parse(dynamicParameters)[1].Value);

        // var $this = qaf.initializeModuleScript(moduleFileName, dataSourceMap);
		// $l.moduleEventLog(moduleID, 'G01', 'ApplicationID {0} - START'.format(applicationID), 'Debug');
    ...
    LF01: (callback, dynamicParameters, dataContext) => {
    ...
    doSomething3: async (moduleID, dynamicParameters, isTransaction, functionSeq) => {
		var requestData = {};
	...
	doSomething4: (callback, moduleID, dynamicParameters, isTransaction, functionSeq) => {
