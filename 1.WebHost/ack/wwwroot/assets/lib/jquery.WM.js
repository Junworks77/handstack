(function ($) {
    ...
    $.fn.WM_close = function () {
        ...
        if (channelID) {
            var itemToFind = syn.$w.iframeChannels.find(function (item) { return item.id == channelID });
            var idx = syn.$w.iframeChannels.indexOf(itemToFind);
            if (idx > -1) {
                syn.$w.iframeChannels.splice(idx, 1);
            }
        }
